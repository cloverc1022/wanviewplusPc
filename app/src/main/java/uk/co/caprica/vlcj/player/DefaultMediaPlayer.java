...
public abstract class DefaultMediaPlayer extends AbstractMediaPlayer implements MediaPlayer, EqualizerListener {
    ...
    public boolean startRecord(String path) {
        logger.debug("startRecord(path={})", path);
        ...
        if (recordDirectory.exists()) {
            int recordStart = libvlc.libvlc_media_player_record_start(mediaPlayerInstance, path);
            logger.debug("recordTaken={}", recordStart);
            return recordStart == 0;
        } else {
            throw new RuntimeException("Directory does not exist and could not be created for '" + path + "'");
        }
    }

    @Override
    public boolean stopRecord() {
        int recordStop = libvlc.libvlc_media_player_record_stop(mediaPlayerInstance);
        logger.debug("recordStop={}", recordStop);
        return recordStop == 0;
    }

    @Override
    public boolean isRecording() {
        boolean isRecording = libvlc.libvlc_media_player_is_recording(mediaPlayerInstance);
    ...
    public boolean isRecordable() {
        boolean isRecordable = libvlc.libvlc_media_player_is_recordable(mediaPlayerInstance);
        logger.debug("isRecordable={}", isRecordable);
        return isRecordable;
    }
}
